    ...
    while True:
        for _ in range(3):
            ...
            if current_city == 'Сдаюсь':
                flag = False
                break
            elif current_city not in city and current_city in cities and current_city[0].lower() in last_letter:
                break
            elif current_city in city:
                print('Уже называли')
            else:
...
def with_computer():
    ...
    while True:
        ...
        if current_city[-1] in 'цыь':
            ...
            last_letter = current_city[-1]        
        suitable_cities = [i for i in cities if i[0].lower() == last_letter]
        if len(suitable_cities) == 0:
            print(f'Конец игры.\nПобедил {player}.')
            break
        print(suitable_cities[0])
        if suitable_cities[0][-1] in 'цыь':
            last_letter = suitable_cities[0][-2]
        else:
            last_letter = suitable_cities[0][-1] 
        cities.remove(suitable_cities[0])
        cities.remove(current_city)
        city.extend([current_city, suitable_cities[0]])
            
mode = input('Как играем, с другом или с компьютером?\n')
